...
$localFolder = "C:\apps\development\powershell\sftp_powshell\downloads"

foreach ($file in $listOfFiles)
{
try
{
Write-Output "Attempting to download $($file) ..`n`n"
Get-SFTPFile -SessionId $ssh.SessionId -RemoteFile $file.FullName -LocalPath $localFolder
Write-Output "Downloaded $($file) ..`n`n"

Write-Output ".. Removing $($file) from source sftp location.`n`n"
Remove-SFTPItem -SessionId $ssh.SessionId -path  $file.FullName -Force
}
