...
foreach ($file in $filteredFiles) {
    ...
    try {
        ...
        # get-sftpitem -SessionId $ssh.SessionId -RemoteFile $file.FullName -LocalPath $localFolder
        # Get-SFTPFile -SessionId $ssh.SessionId -RemoteFile $file.FullName -LocalPath $localFolder
        Get-SFTPitem -SessionId $ssh.SessionId -Path $file.FullName -Destination $localFolder
